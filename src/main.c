...
void handle_syscall_io(int pid, int i) {
    ...
    if (io.iov_len == sizeof(regs.regs64)) {
        ...
        if (!(i % 2)) {
            ...
            struct user_regs_struct current_regs = regs.regs64;
            syscall_x86_64_t syscall = syscalls_x86_64[current_regs.orig_rax];
            fprintf(stderr, syscall.format, syscall.name, current_regs.rdi, current_regs.rsi, current_regs.rdx,
                    current_regs.rcx, current_regs.r8, current_regs.r9);
        } else {
            fprintf(stderr, ") = %llu\n", regs.regs64.rax);
        }
    } // else {
...
int trace_syscalls(int pid) {
    ...
    disable_signals();
    // child wait when parent is ready
    waitpid(pid, 0, 0);
    for (int i = 0; 42; i++) {
        status = 0;
